...
namespace Utils
{
	...
	bool String::BeginsWith(std::string str, std::string beginStr)
	{
		return str.compare(0, beginStr.size(), beginStr) == 0;
	}
	...
	StringArray String::GetArrayOfBetween(const std::string &strSource, const std::string &strBefore, const std::string &strAfter, unsigned int nPosDebut, unsigned int nPosFin)
	{
		...
		{
			LOGASSERT(false, "End position must be after start position");
		}
		else
		{
